...
$(board).on('scoreUpdate', function(e, info) {
    ...
    } else if (info.score >= 0 && info.score < 50) {
        $(level).empty().append("Great Start");
        if (info.score >= 0 && info.score < 3) {
            new Audio('/graphics/2.mp3').play();
        }

    } else if (info.score > 50 && info.score <= 100) {
        $(level).empty().append("Noobie!");
        if (info.score >= 51 && info.score < 53 || info.score >= 97 && info.score < 100) {
            new Audio('/graphics/3.mp3').play();
        }
    } else if (info.score > 100 && info.score <= 150) {
        $(level).empty().append("Cool!");
        if (info.score >= 100 && info.score < 104 || info.score >= 146 && info.score < 149) {
            new Audio('/graphics/4.mp3').play();
        }
    } else if (info.score > 150 && info.score <= 200) {
        $(level).empty().append("Beginner!");
        if (info.score >= 150 && info.score < 154 || info.score >= 197 && info.score < 200) {
            new Audio('/graphics/5.mp3').play();
        }
    } else if (info.score > 200 && info.score <= 250) {
        $(level).empty().append("Pro Player!");
        if (info.score >= 200 && info.score < 204 || info.score >= 247 && info.score < 250) {
            new Audio('/graphics/6.mp3').play();
        }
    } else if (info.score > 250 && info.score <= 300) {
        $(level).empty().append("Expert!");
        if (info.score >= 250 && info.score < 254 || info.score >= 297 && info.score < 300) {
            new Audio('/graphics/7.mp3').play();
        }
    } else if (info.score > 300 && info.score <= 350) {
        $(level).empty().append("Legend!");
        if (info.score >= 300 && info.score < 304 || info.score >= 347 && info.score < 350) {
            new Audio('/graphics/8.mp3').play();
        }
    } else if (info.score > 350 && info.score <= 400) {
        $(level).empty().append("UltraExperto!");
        if (info.score >= 350 && info.score < 353 || info.score >= 396 && info.score < 400) {
            new Audio('/graphics/9.mp3').play();
        }
    }
    // parseInt(localStorage.getItem('num')) + 1
    //let countvalue = JSON.parse(localStorage.getItem('count-value'));

    //let countvalue = parseInt(localStorage.getItem('count-value'))
