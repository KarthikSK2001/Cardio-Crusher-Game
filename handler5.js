...
$(board).on('scoreUpdate', function(e, info) {
    ...
    if (info.score >= 1000) {
        ...
        Swal.fire({
            ...
            backdrop: `
              rgba(0,0,123,0.4)
              url("/graphics/success.gif")
              center top
              no-repeat
            `
        }).then(function() {
            window.location = "level_list.html";
    ...
    if (info.score == 0) {
        $(level).empty().append("Lets Start (Kid)");
        new Audio('/graphics/1.mp3').play();
    } else if (info.score >= 0 && info.score < 100) {
        $(level).empty().append("Great Start");
        if (info.score >= 0 && info.score < 3) {
            new Audio('/graphics/2.mp3').play();
        }

    } else if (info.score > 100 && info.score <= 200) {
        $(level).empty().append("Noobie!");
        if (info.score >= 100 && info.score < 103 || info.score >= 196 && info.score < 200) {
            new Audio('/graphics/3.mp3').play();
        }
    } else if (info.score > 200 && info.score <= 300) {
        $(level).empty().append("Cool!");
        if (info.score >= 200 && info.score < 204 || info.score >= 296 && info.score < 300) {
            new Audio('/graphics/4.mp3').play();
        }
    } else if (info.score > 300 && info.score <= 400) {
        $(level).empty().append("Beginner!");
        if (info.score >= 300 && info.score < 303 || info.score >= 397 && info.score < 400) {
            new Audio('/graphics/5.mp3').play();
        }
    } else if (info.score > 400 && info.score <= 500) {
        $(level).empty().append("Pro Player!");
        if (info.score >= 400 && info.score < 404 || info.score >= 497 && info.score < 500) {
            new Audio('/graphics/6.mp3').play();
        }
    } else if (info.score > 500 && info.score <= 600) {
        $(level).empty().append("Expert!");
        if (info.score >= 500 && info.score < 504 || info.score >= 596 && info.score < 600) {
            new Audio('/graphics/7.mp3').play();
        }
    } else if (info.score >= 600 && info.score <= 700) {
        $(level).empty().append("Legend!");
        if (info.score >= 600 && info.score < 604 || info.score >= 697 && info.score < 700) {
            new Audio('/graphics/8.mp3').play();
        }
    } else if (info.score > 700 && info.score <= 800) {
        $(level).empty().append("UltraExperto!");
        if (info.score >= 700 && info.score < 704 || info.score >= 796 && info.score < 800) {
            new Audio('/graphics/9.mp3').play();
        }
    } else if (info.score > 800 && info.score <= 1000) {
        $(level).empty().append("The Venom!");
        if (info.score >= 800 && info.score < 804 || info.score >= 996 && info.score < 1000) {
            new Audio('/graphics/10.mp3').play();
        }
    }
