...
$(board).on('scoreUpdate', function(e, info) {
    ...
    } else if (info.score > 400 && info.score <= 450) {
        $(level).empty().append("Pro Player!");
        if (info.score >= 400 && info.score < 404 || info.score >= 447 && info.score < 450) {
            new Audio('/graphics/6.mp3').play();
        }
    } else if (info.score > 450 && info.score <= 500) {
        $(level).empty().append("Expert!");
        if (info.score >= 450 && info.score < 454 || info.score >= 497 && info.score < 500) {
            new Audio('/graphics/7.mp3').play();
        }
    } else if (info.score > 500 && info.score <= 550) {
        $(level).empty().append("Legend!");
        if (info.score >= 500 && info.score < 504 || info.score >= 547 && info.score < 550) {
            new Audio('/graphics/8.mp3').play();
        }
    } else if (info.score > 550 && info.score <= 600) {
        $(level).empty().append("UltraExperto!");
        if (info.score >= 550 && info.score < 553 || info.score >= 596 && info.score < 600) {
